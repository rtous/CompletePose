...
NEURONS_PER_LAYER_GENERATOR = 256 #256
BIAS = False
class Generator(nn.Module):
    def __init__(self, ngpu, numJoints, nz):
        super(Generator, self).__init__()
        ...
        self.main = nn.Sequential(
          ...
          nn.Linear(NEURONS_PER_LAYER_GENERATOR, NEURONS_PER_LAYER_GENERATOR//2, bias=False),
          nn.BatchNorm1d(NEURONS_PER_LAYER_GENERATOR//2, 0.8),
          nn.LeakyReLU(0.25),

          nn.Linear(NEURONS_PER_LAYER_GENERATOR//2, NEURONS_PER_LAYER_GENERATOR, bias=False),
          nn.BatchNorm1d(NEURONS_PER_LAYER_GENERATOR, 0.8),
          nn.LeakyReLU(0.25),
...
class Discriminator(nn.Module):
  def __init__(self, ngpu, numJoints, nz):
    ...
    self.nz = nz
    self.main = nn.Sequential(
        nn.Linear(self.image_size*2, NEURONS_PER_LAYER_GENERATOR, bias=BIAS),
        nn.BatchNorm1d(NEURONS_PER_LAYER_GENERATOR, 0.8),
        nn.LeakyReLU(0.2, inplace=True),
        #nn.Dropout(0.3),
  ...
  def forward(self, batch_of_keypoints_cropped, batch_of_keypoints_original):#noise not used here but for compatibility with others
...
def trainStep(models, trainSetup, b_size, device, tb, step_absolute, num_epochs, epoch, i, batch_of_keypoints_cropped, batch_of_keypoints_original, confidence_values, scaleFactor, x_displacement, y_displacement, batch_of_json_file):
    '''
    1) pass a batch of real samples (original, cropped) to D with label "real"
    2) G(noide, cropped) -> fake poses
    3) pass the fake samples to D with label "fake" -> estimated labels
    4) train G with the estimated labels
    ''' 
...
def inference(models, b_size, noise, numJoints, batch_of_keypoints_cropped, confidence_values):
    ...
    with torch.no_grad():
        fake = models.netG(batch_of_keypoints_cropped, noise).detach().cpu()
